#!/usr/bin/env python3
"""Generate a sample steel member-schedule PDF for testing Anu-AI.

Requires reportlab (a dev-only dependency, not part of requirements.txt):
...
# Header uses a unit-less "Length" column so per-cell units (m / mm) are honoured.
DATA = [
...
def main() -> None:
    ...
    # Free-text member line to exercise the text parser too.
    story.append(
        Paragraph(
            "Additional: B2 ISMB 200 Length 5.0 m Qty 3 Grid A-2 EL 12.5",
            styles["Normal"],
        )
    )
