...
STEEL_DENSITY_KG_PER_MM3 = 7.85e-6  # 7850 kg/m^3 per mm^3

ISMB_WEIGHTS: dict[int, float] = {
    100: 11.5, 125: 13.0, 150: 15.0, 175: 19.3, 200: 25.4, 225: 31.2,
    250: 37.3, 300: 44.2, 350: 52.4, 400: 61.6, 450: 72.4, 500: 86.9,
    550: 103.7, 600: 122.6,
}

ISMC_WEIGHTS: dict[int, float] = {
    75: 7.14, 100: 9.56, 125: 13.1, 150: 16.4, 175: 19.1, 200: 22.1,
    225: 25.9, 250: 30.4, 300: 35.8, 400: 49.4,
}

SECTION_TYPE_ALIASES = {
    "ISMB": "ISMB", "MB": "ISMB", "ISMC": "ISMC", "MC": "ISMC",
    "RHS": "RHS", "SHS": "RHS", "PL": "PL", "PLATE": "PL", "PLT": "PL",
}

_NOMINAL_RE = re.compile(r"(\d{2,4})")
_RHS_RE = re.compile(r"(\d+(?:\.\d+)?)\s*[xX*×]\s*(\d+(?:\.\d+)?)\s*[xX*×]\s*(\d+(?:\.\d+)?)")
_PL_RE = re.compile(
    r"(?:PL|PLATE|PLT)\s*[.:]?\s*(\d+(?:\.\d+)?)\s*[xX*×]\s*(\d+(?:\.\d+)?)", re.IGNORECASE
)
...
def unit_weight_kg_per_m(section: str) -> float | None:
    ...
    if family in ("ISMB", "ISMC"):
        ...
        return table.get(int(match.group(1)))
    if family == "RHS":
...
def plate_weight_kg(section: str, length_mm: float | None) -> float | None:
    ...
    return width_mm * thickness_mm * length_mm * STEEL_DENSITY_KG_PER_MM3
